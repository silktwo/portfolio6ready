...
export async function POST(request: NextRequest) {
  try {
    ...
    const adapter = adapters.strapi
    if (!adapter.isAvailable()) {
      return NextResponse.json({ message: 'Strapi adapter not available' }, { status: 400 })
    }
    ...
    console.log(`🪝 Strapi webhook processed: ${actions.join(', ')}`)
